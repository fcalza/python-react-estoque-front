...
const EntradaProduto = () => {
  ...
  const fetchLogs = async () => {
    try {
      const result = await axios("http://127.0.0.1:5000/log_produto/entradas");
      setLogs(result.data);
    } catch (err) {
      console.log(err);
  ...
  const handleEntrada = async (e) => {
    ...
    try {
      ...
      await axios.post(
        `http://127.0.0.1:5000/produto/entrada/${selectedProduto}`,
      ...
      setError('Erro ao processar a entrada do produto. Tente novamente mais tarde.');
    }

  }

  return (
    <div className="container entrada-produto">
      <h3 className='titulo_tela'>Entrada de Produto</h3>
      <form onSubmit={handleEntrada}>
        ...
        <button type="submit" className="btn btn-success mt-3">Registrar Entrada</button>
      </form>
      <div className="mt-5">
